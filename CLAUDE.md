...
- **`landed_cost`** (`LandedCost`) — себестоимость единицы номенклатуры, доведённой до склада (BYN):
  ...
  - Фиксируется на приёмке заказа через `_fixate_landed_cost`: агрегирует позиции по `sku_code`,
    разносит `freight_byn` (общий движок `allocate_landed_cost`, база — вес, иначе стоимость), затем
    добавляет пошлину ТН ВЭД per-SKU (% из `sku_master.landed_inputs` на дату приёмки, к таможенной
    стоимости goods+фрахт ПОСЛЕ разнесения — тот же путь, что в плановой оценке) → одна строка на
    номенклатуру, `stage="actual"` (реальная приёмка; плановая cost-estimate — `estimated`). Два FX /
    буфер +10% — Горизонт 2.
...
- `GET/POST /procurement/organizations/{org_id}/claims` — журнал и создание ручной претензии
  ...
  Для претензии ОТК без юрлица закупщик сначала указывает точные ссылку `production:qc:ID`,
  изделие, причину и номер наряда в `POST .../unassigned-claim-preview`, затем выбирает
  активного MDM-поставщика и передаёт проверенный digest в `POST .../claims/{id}/bind`.
  Сопоставление сохраняет исходный снимок ОТК и основание принадлежности; повтор той же UUID-команды
  не создаёт дубль. Запись с уже указанным историческим поставщиком требует отдельной сверки.
...
- Общий `/suppliers/{id}/scorecard` закрыт (410): он смешивал данные юрлиц. Показатели
  доступны только через `/organizations/{org_id}/suppliers/{id}/scorecard` после проверки доступа.
  Претензии и общий балл не рассчитываются до подтверждённой принадлежности претензий юрлицу.
- Новый `/organizations/{org_id}/claims` создаёт ручную претензию с подтверждённым MDM ID
  поставщика и неизменяемым основанием владения; `order_id`, если задан, сверяется с юрлицом
  и поставщиком. Повтор использует UUID команды. `/resolve` сохраняет автора, время и до/после;
  его событие `procurement.claim.resolution_recorded` не создаёт платёж без банковского основания.
  Исторические и автоматические претензии без юрлица не назначаются автоматически. Общие
  `/claims` закрыты (410); экран закупок использует только маршрут выбранного юрлица.
- Карточка авто-заявки
...
- **landed cost — мин.срез:** позиция без `sku_code` или с `qty<=0` пропускается (себестоимость
  единицы не определена — не пишем `unit=0`, чтобы не замаскировать дыру в марже); издержки =
  `freight_byn` (база — вес, иначе стоимость) + пошлина ТН ВЭД per-SKU (% из `sku_master.landed_inputs`
  на дату приёмки, к таможенной стоимости goods+фрахт после разнесения). Два FX-курса и буфер +10%
  (методика «Расчёт Китай») — Горизонт 2. Себестоимость храним в 4 знака, единицу квантуем в 4 знака
  (`Numeric(14,4)`); таможенный `total_landed_byn` в payload — с пошлиной (finance читает total).
  **Результат — в СВОЮ таблицу `procurement.landed_cost`, НЕ в
...
- **Факт важнее оценки в фасаде:** `last_landed_cost`/`_batch` сортируют `actual`→`estimated` (затем свежее
  ...
  No-op на данных, где только `actual` (вся текущая боевая). Оценка и факт теперь оба ВКЛЮЧАЮТ пошлину
  ТН ВЭД (план — в `_recompute_estimated_landed`, факт — в `_fixate_landed_cost`), поэтому после приёмки
  себестоимость не «падает» на величину пошлины — план и факт сходятся (маржа sales не завышается).
